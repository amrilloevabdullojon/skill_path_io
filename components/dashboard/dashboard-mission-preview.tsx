...
function missionStatusMeta(status: string) {
  ...
  if (normalized === "locked") {
    return {
      label: "Locked",
      chip: "border-border bg-card/70 text-muted-foreground",
      card: "border-border/80 bg-card/45",
            ...
            <article key={mission.id} className={cn("surface-panel-hover rounded-xl border p-3.5", statusMeta.card)}>
              <div className="flex items-start justify-between gap-3">
                <div className="min-w-0">
                  ...
                  <p className="mt-0.5 line-clamp-2 text-xs leading-5 text-muted-foreground">{mission.scenario}</p>
                </div>
