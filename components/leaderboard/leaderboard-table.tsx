        ...
        <tbody>
          {rows.map((row) => (
            <tr key={row.userId} className="table-row">
              <td className="inline-flex items-center gap-2 px-3 py-3 font-semibold text-foreground">
                {rankIcon(row.rank)}
                {row.rank}
              </td>
              ...
              {!compact && <td className="px-3 py-3 text-muted-foreground">{row.completedModules}</td>}
