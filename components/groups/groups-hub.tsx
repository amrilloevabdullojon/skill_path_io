  ...
  return (
    <section className="space-y-5">
      <header className="surface-elevated space-y-2 p-5 sm:p-6">
        <p className="kicker">Группы</p>
        <h1 className="page-title">Учебные группы и совместная работа</h1>
        <p className="section-description">Вступай в группы по темам, обсуждай модули и проводи сессии взаимного обучения.</p>
      </header>
          ...
          return (
            <article key={group.id} className="surface-elevated space-y-3 p-4">
              <div className="flex items-start justify-between gap-3">
                ...
                >
                  {joined ? "Вступил" : "Вступить"}
                </button>
              </div>
              <p className="text-sm text-muted-foreground">{group.description}</p>
              <p className="inline-flex items-center gap-2 text-xs text-muted-foreground">
                <UsersRound className="h-3.5 w-3.5" />
                {group.members + (joined ? 1 : 0)} участников · {group.mode}
              </p>
            </article>
          );
        })}
