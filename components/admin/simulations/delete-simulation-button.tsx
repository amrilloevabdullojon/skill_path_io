...
import { deleteSimulationAction } from "@/app/admin/actions";
...
export function DeleteSimulationButton({
  ...
  function handleDelete() {
    if (!confirm(`Delete "${simulationTitle}"?\n\nThis cannot be undone.`)) return;
    const formData = new FormData();
    formData.set("simulationId", simulationId);
    startTransition(() => {
      deleteSimulationAction(formData);
    });
  }
  ...
  return (
    <button
      ...
      aria-label={`Delete ${simulationTitle}`}
