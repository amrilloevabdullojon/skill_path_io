  ...
  return (
    <div className="space-y-3">
      ...
      <div className="flex items-center justify-end gap-2 text-[11px] text-muted-foreground">
        ...
        <span className="h-2.5 w-2.5 rounded bg-indigo-900/80" />
        <span className="h-2.5 w-2.5 rounded bg-indigo-700/80" />
        <span className="h-2.5 w-2.5 rounded bg-cyan-500/80" />
        <span className="h-2.5 w-2.5 rounded bg-emerald-400/90" />
        <span>Больше</span>
      </div>
      <p className="text-[11px] text-muted-foreground">Оранжевое кольцо обозначает активную серию обучения.</p>
    </div>
  );
}
