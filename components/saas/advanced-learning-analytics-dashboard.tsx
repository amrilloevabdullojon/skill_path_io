...
export function AdvancedLearningAnalyticsDashboard({ analytics }: AdvancedLearningAnalyticsDashboardProps) {
  ...
  const currentSkillGrowth = analytics.skillGrowth.current;
  ...
  return (
    <section className="space-y-6">
      <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-6 surface-hero border border-indigo-500/20 bg-card p-6 sm:p-8 rounded-2xl">
        <div className="space-y-2 relative z-10 w-full lg:w-2/3">
          ...
          <h1 className="page-title tracking-tight text-foreground">Динамика обучения и навыки</h1>
      ...
      <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-5">
        {[
          ...
          <article key={idx} className={`surface-elevated border bg-card p-5 rounded-2xl relative overflow-hidden transition-all hover:-translate-y-1 ${stat.isSkill ? "border-indigo-500/40 bg-gradient-to-br from-indigo-500/10 to-transparent" : "border-border/50"}`}>
      ...
      <div className="grid gap-6 xl:grid-cols-2">
        <article className="surface-elevated border border-border/50 bg-card p-6 rounded-2xl">
